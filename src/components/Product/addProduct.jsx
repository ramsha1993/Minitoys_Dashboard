...
export default function AddProduct() {
  ...
  const handleThumbs = (e) => {
    ...
    files.forEach((file) => {
      const reader = new FileReader();
      reader.onload = (ev) =>
        setThumbs((prev) => [...prev, ev.target.result]);
      reader.readAsDataURL(file);
    });
  };
    ...
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <header className="sticky top-0 z-10 bg-white border-b border-gray-100 px-6 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            className="text-sm px-4 py-1.5 rounded-xl bg-gray-900 text-white font-medium hover:bg-gray-700 transition"
          >
            Publish product
          </button>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-6 grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-5">
        ...
        <div className="flex flex-col gap-5">
          ...
          <Card title="Hero image">
            <label
              className={`relative flex flex-col items-center justify-center gap-2 border-2 border-dashed rounded-2xl cursor-pointer transition overflow-hidden ${
                heroPreview
                  ? "border-gray-200"
          ...
          <Card title="Status">
            {STATUS_OPTIONS.map((opt) => (
              <div
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border cursor-pointer transition mb-2 last:mb-0 ${
                  status === opt.value
